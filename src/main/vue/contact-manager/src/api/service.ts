...
const fetchRequests = (offset: number, limit: number) => axios.get(`api/v2/contacts?offset=${offset}&limit=${limit}`);
const removeRequest = (id: string) => axios.delete(`api/v2/contacts/${id}`);
const updateRequest = (id: string, request: Request) => axios.put(`api/v2/contacts/${id}`, {
...
const addRecipient = (id: string, recipient: ContactPerson) => axios.post(`api/v2/contacts/${id}/recipients`, recipient);
const updateRecipient = (id: string, personId: string, recipient: ContactPerson) => {
  axios.put(`api/v2/contacts/${id}/recipients/${personId}`, {
    name: recipient.name,
...
const removeRecipient = (id: string, personId: string) => axios.delete(`api/v2/contacts/${id}/recipients/${personId}`);
const getRequest = (id: string) => axios.get(`api/v2/contacts/${id}`);
